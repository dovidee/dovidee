<div align=center>
  
# dovidee

</div>

<p align="center">
  <img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExZ3RobjB3ZHMxcGp5enZjNTF3NTBwZnhrZmg1dGIzejFlZXRwbnk5aCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/TRebCjNbc4dIA/giphy.gif" width="600" alt="System Analysis Animation">
</p>

### 📡 Status
![Active](https://img.shields.io/badge/Status-Active-success?style=flat-square) 
![Role](https://img.shields.io/badge/Focus-Systems_Security-blue?style=flat-square)
![OS](https://img.shields.io/badge/Env-Linux%20%7C%20Windows-lightgrey?style=flat-square)

---

### 🎓 Education
* **B.S. Information Technology** - Second year (In progress)
* **HTB Certified Penetration Testing Specialist (CPTS)** – 100% completed (Exam candidate)

### 🛠️ Technical
* **Languages:** `Pyret` `Python` `C#` `Bash`
* **Environments:** Linux (Debian/Arch), Windows Server (AD/GPO), Docker, Virtualization (KVM/VMware)

### 🔬 Labs
* **Internal Maneuvering:** Exploring pivoting techniques and lateral movement.
* **Access Logic:** Access control and IDOR vulnerabilities.
* **Identity:** Active Directory attacks in lab environments (HTB/THM).

### 🚀 Projects
* **[Vigil](https://github.com/dovidee/vigil)** – Modular, high-concurrency reconnaissance framework with stateful delta-tracking. 
* **[Kartverket Project](https://github.com/letsgorm/KartverketProject)** – Obstacle registration system for pilots.

---

### 📉 Activity
<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=dovidee&show_icons=true&theme=dark&hide_border=true&bg_color=00000000" alt="Stats" width="400">
</p>
